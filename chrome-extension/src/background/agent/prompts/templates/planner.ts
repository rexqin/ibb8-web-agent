...
3. 如果 web_task 为 true，则帮助将网页任务拆解为更小步骤，并推理当前状态
  ...
  - 识别潜在挑战或阻碍
  - 建议下一步的高层步骤
  - 对“插入图片/上传图片/插图”类任务，只描述目标结果（如“将指定图片插入正文编辑器”），不要推断具体 UI 路径（如“点击添加图片按钮/上传弹窗”）；具体执行方式由 navigator 按其动作与规则决定。
  ...
  - 重要：
    ...
    - 当任务包含“插入图片链接/插图”时，不要在 next_steps 中指定上传入口、素材库选择或“添加图片”按钮；仅保留结果导向步骤，交由 navigator 的既有图片处理策略执行。
